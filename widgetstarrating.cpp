...
void StarRating::initStarPolygon()
{
    starPolygon.clear();
    starPolygon << QPointF(1.0, 0.5);
    for (int i = 1; i < 5; ++i)
        starPolygon << QPointF(0.5 + 0.5 * std::cos(0.8 * i * M_PI),
                               0.5 + 0.5 * std::sin(0.8 * i * M_PI));
}
                       ...
                       const QPalette &palette, EditMode mode) const
{
    ...
    for (int i = 0; i < myMaxStarCount; ++i) {
        if (i < myStarCount)
            ...
            painter->drawPolygon(QPolygonF() << QPointF(0.4, 0.5) << QPointF(0.5, 0.4)
